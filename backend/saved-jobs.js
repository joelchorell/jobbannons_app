...
function setupSaveJobButton() {
  ...
  saveJobBtn.addEventListener("click", async function () {
    ...
    try {
      showLoadingOverlay("Sparar jobbannons...");
...
function setupLoadJobsButton() {
  document.addEventListener("click", async function (e) {
    if (e.target && e.target.id === "loadJobsBtn") {
      ...
      try {
        showLoadingOverlay("Hämtar sparade annonser...");
...
async function loadSavedJob(jobId) {
  try {
    showLoadingOverlay("Laddar jobbannons...");
...
async function deleteSavedJob(jobId) {
  try {
    ...
    showError(`Kunde inte ta bort annonsen: ${error.message}`);
    return false;
  }
}
...
function showSuccess(message) {
  alert(message); // Replace with your success message UI
}
